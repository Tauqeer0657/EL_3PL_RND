...
import { styled } from "@mui/material/styles";
import { _getAll_WithoutToken } from "../../../commonUtilApi/ElApi";


const AccessTable = ({ USER_CD }) => {
  const [data, setData] = useState([]);
  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [permissions, setPermissions] = useState({});
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await _getAll_WithoutToken("api/access/");
        setData(response.data);
        ...
        if (USER_CD) {
          ...
          const initialState = response.data.reduce((acc, item, index) => {
            const userAccessItem = userAccessData.find(
              (ua) => ua.pageCode === item.pageCode
            );
            acc[index] = {
              page_YN: userAccessItem ? userAccessItem.page_YN === "Y" : false,
              page_Inq: userAccessItem ? userAccessItem.page_Inq === "Y" : false,
    ...
    <Box>
      ...
      >
        ...
        >
          <TableHead>
            <TableRow style={{ backgroundColor: "#f5f5f5" }}>
              <TableCell style={{ fontWeight: "bold" }}>Page Code</TableCell>
